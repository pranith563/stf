...
module.exports.builder = function(yargs) {
  ...
  return yargs
    ...
    .option('duplicate-serial-fix', {
      describe: 'Normalize known duplicate ADB serials before spawning ' +
        'device workers. Negate with --no-duplicate-serial-fix.'
    , type: 'boolean'
    , default: true
    })
    .option('duplicate-serial-value', {
      describe: 'ADB serial value that should be treated as duplicate/broken.'
    , type: 'string'
    , default: '0123456789ABCDEF'
    })
    .option('duplicate-serial-prefix', {
      describe: 'Prefix for generated ADB serials.'
    , type: 'string'
    , default: 'MTKADB'
    })
    .option('duplicate-serial-width', {
      describe: 'Numeric width for generated ADB serials.'
    , type: 'number'
    , default: 3
    })
    .option('duplicate-serial-state-file', {
      describe: 'Local provider state file for USB path to generated serial ' +
        'assignments.'
    , type: 'string'
    , default: '/var/lib/devicefarmer/duplicate-serial-map.json'
    })
    .option('duplicate-serial-require-mtk', {
      describe: 'Only normalize duplicate serial devices that expose MTK-like ' +
        'Android properties. Negate with --no-duplicate-serial-require-mtk.'
    , type: 'boolean'
    , default: true
    })
...
module.exports.handler = function(argv) {
  ...
  function run() {
    ...
    , procutil.fork(path.resolve(__dirname, '..'), [
        ...
        , '--duplicate-serial-value', argv.duplicateSerialValue
        , '--duplicate-serial-prefix', argv.duplicateSerialPrefix
        , '--duplicate-serial-width', argv.duplicateSerialWidth
        , '--duplicate-serial-state-file', argv.duplicateSerialStateFile
        ]
        .concat(argv.allowRemote ? ['--allow-remote'] : [])
        .concat(!argv.duplicateSerialFix ? ['--no-duplicate-serial-fix'] : [])
        .concat(!argv.duplicateSerialRequireMtk ?
          ['--no-duplicate-serial-require-mtk'] : [])
