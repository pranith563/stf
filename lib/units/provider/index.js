...
var createIdentityNormalizer = require('./identity-normalizer')
...
module.exports = function(options) {
  ...
  var identityNormalizer = createIdentityNormalizer(options, log)
  var workers = {}
  ...
  client.trackDevices().then(function(tracker) {
    ...
    tracker.on('add', function(device) {
      identityNormalizer.normalize(device)
        .then(function(suppressed) {
          if (suppressed) {
            return false
          }

          return filterDevice(addDevice)(device)
        })
        .catch(function(err) {
          log.error(
            'Identity normalization failed for "%s": %s'
          , device.id
          , err.message
          )
        })
    ...
    tracker.on('change', function(device) {
      identityNormalizer.normalize(device)
        .then(function(suppressed) {
          ...
          return filterDevice(function(filteredDevice) {
            flippedTracker.emit(filteredDevice.id, 'change', filteredDevice)
          })(device)
        })
        .catch(function(err) {
          log.error(
