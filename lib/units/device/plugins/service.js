...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, router, push, sdk, apk) {
    ...
    function callService(intent) {
      var startServiceCmd = (sdk.level < 26 || sdk.level >= 33) ?
    ...
    function shellAndDrain(command, timeout) {
    ...
    function bringToForeground() {
      log.info('Waking device and bringing STFService app to foreground')
      return shellAndDrain('input keyevent KEYCODE_WAKEUP', 5000)
        .then(function() {
          return Promise.delay(500)
        })
        .then(function() {
          return shellAndDrain('input keyevent KEYCODE_MENU', 5000)
        })
        .then(function() {
          return adb.shell(options.serial, util.format(
            "am start --user 0 -a '%s' -d package:%s"
          , 'jp.co.cyberagent.stf.ACTION_ICON'
          , apk.pkg
          ))
            .timeout(15000)
        })
    ...
    function openService() {
      ...
      var launchPromise = Promise.resolve(true)

      if (sdk.level >= 33) {
        launchPromise = bringToForeground()
      }
